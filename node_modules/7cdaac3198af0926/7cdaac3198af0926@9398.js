...
md`# Amplia Saúde - Trilhas Exploratórias - v4.0 - Características do Nascimento`
)}
...
function _btn_adicionar_municipio(d3,municipios,sel_municipios,$0,colors,cache,$1,data,fetchPM25,fetchPESO,fetchRobson,procPM25,procPESO,procRobson){return(
d3.create("button").attr("class","btn-adiciona").text("+").on("click",e=>{
  if (!municipios.map(d=>d.municipio).includes(document.getElementById("menu-municipios").value))
    return
  let municipio=document.getElementById("menu-municipios").value
  document.getElementById("menu-municipios").value="Adicione município"
  if (sel_municipios.length==6) return
  if (sel_municipios.map(d=>d.nome).includes(municipio)) return
  
  $0.value=$0.value.concat([{nome:municipio,cor:colors.find(d=>!sel_municipios.map(d=>d.cor).includes(d))}])
  
  if (cache.map(d=>d.municipio).includes(municipio)){
    $1.value=data.concat(cache.filter(d=>d.municipio==municipio))
    return
  }  
  Promise.all([
  fetchPM25(municipio),
  fetchPESO(municipio),
  fetchRobson(municipio)
  ]).then(v=>{
    $1.value=$1.value.concat([{municipio:municipio,variavel:"PM25",dados:procPM25(v[0])}, {municipio:municipio,variavel:"PESO",dados:procPESO(v[1])},{municipio:municipio,variavel:"ROBSON",dados:procRobson(v[2])}])
  })
}).node()
)}
...
function _funcoesGeradoras(gPM25,gPESO,gROBSON,gTOTAL){return(
{PM25:gPM25,PESO:gPESO,ROBSON:gROBSON,TOTAL:gTOTAL}
)}

function _funcoesProc(procPM25,procPESO,procRobson,procTOTAL){return(
{PM25:procPM25,PESO:procPESO,ROBSON:procRobson,TOTAL:procTOTAL}
)}

function _funcoesFetch(fetchPM25,fetchPESO,fetchRobson,fetchTOTAL){return(
{PM25:fetchPM25,PESO:fetchPESO,ROBSON:fetchRobson,TOTAL:fetchTOTAL}
)}
...
function _gPESO(sel_municipios,data,gLoadingTrilha,conf_trilhas,mp,Plot,width,height,semanas_epidemiologicas,intervalo,filterDate,janela,lineWidthFiltered,lineWidth,bisector,sel,legendTooltip,$0,d3,mouseDisplace,html,corTrilha,removeTrilha,copy_button,checkMediaMovel,checkTempo,inner_slider,checkFiltros){return(
(idx) =>
{
  ...
  legendTooltip(g,'PESO',idx);
  g.onmousedown=(e)=>{$0.value=Math.min(Math.max(g.scale("x").invert(e.offsetX),g.scale("x").domain[0]),g.scale("x").domain[1])}
  d3.select(g).call(d3.drag().on("drag",(e)=>$0.value=Math.min(Math.max(g.scale("x").invert(e.x-mouseDisplace(e.x)),g.scale("x").domain[0]),g.scale("x").domain[1])))
  
  return html`
<div style="--cor-trilha: ${corTrilha["PESO"]}; display:inline-flex; align-items:center; justify-content:space-between;">
<div class="buttons">
  ${d3.create("button").attr("class","btn-remove").text("–").on("click",removeTrilha).node()}<br>
  ${copy_button()}
</div>
...
function _gROBSON(sel_municipios,data,gLoadingTrilha,conf_trilhas,mp,Plot,width,height,semanas_epidemiologicas,intervalo,filterDate,janela,lineWidthFiltered,lineWidth,bisector,sel,legendTooltip,$0,d3,mouseDisplace,html,corTrilha,removeTrilha,copy_button,checkRobson,checkMediaMovel,checkTempo,checkEscala,inner_slider,checkFiltros){return(
(idx) => {
  if(!sel_municipios.map(d=>d.nome).every(d=>data.filter(e=>e.idx==idx).map(e=>e.municipio).includes(d)))
    return gLoadingTrilha()
  ...
  let robson=conf_trilhas.find(d=>d.i==idx).robson
  let g=
    Plot.plot({
    marginTop:5,
    marginRight:0,
...
data.filter(d=>d.idx==idx).map((d,i)=>Plot.line(d.dados.filter(d=>filterDate(d,idx)),Plot.windowY({
  ...
  strokeDasharray:d.filtrado?"3":"1"
      ...
      :data.filter(d=>d.idx==idx).map((d,i)=>Plot.line(d.dados.filter(d=>filterDate(d,idx)), {
        ...
        Plot.ruleX([data.filter(d=>d.idx==idx)[0].dados[bisector.center(data.filter(d=>d.idx==idx)[0].dados,sel)]],{x:"x",strokeDasharray:"5"}),
        !conf_trilhas.find(d=>d.i==idx).mostra_media_movel?data.filter(d=>d.idx==idx).map((d,i)=>Plot.dot([d.dados[bisector.center(d.dados,sel)]],{x:"x",y:d=>d[escala][robson],fill:sel_municipios.find(e=>e.nome==d.municipio).cor})):null
      ]
    })
  legendTooltip(g,'ROBSON',idx,escala,robson);
  g.onmousedown=(e)=>{$0.value=Math.min(Math.max(g.scale("x").invert(e.offsetX),g.scale("x").domain[0]),g.scale("x").domain[1])}
  d3.select(g).call(d3.drag().on("drag",(e)=>$0.value=Math.min(Math.max(g.scale("x").invert(e.x-mouseDisplace(e.x)),g.scale("x").domain[0]),g.scale("x").domain[1])))

    return html`
<div style="--cor-trilha: ${corTrilha["ROBSON"]}; display:inline-flex; align-items:center; justify-content:space-between;">
<div class="buttons">
  ${d3.create("button").attr("class","btn-remove").text("–").on("click",removeTrilha).node()}<br>
  ${copy_button()}
</div>
...
<div class="trilha" i=${idx}>
  <div class="cabecalho-trilha">
    ...
    ${(()=>{let s=d3.create("select").attr("class","menuIG");
            ...
            s.append("option").attr("name","Robson1a4").text("Robson1a4").property("selected",conf_trilhas.find(d=>d.i==idx).robson=="Robson1a4");
            s.append("option").attr("name","Robson5").text("Robson5").property("selected",conf_trilhas.find(d=>d.i==idx).robson=="Robson5");
            s.append("option").attr("name","Robson6a10").text("Robson6a10").property("selected",conf_trilhas.find(d=>d.i==idx).robson=="Robson6a10");
            s.on("change",e=>checkRobson(e,idx));
            return s.node();}
      )()}
    <div class="opcoes-trilha">
...
<div>
<center><span class="fonte">Fonte: ${variavel=="PM25"?"SISAM":"SINASC"}<span></center>
</div>
</div>`
...
<span class="fonte">Fonte: ${variavel=="PM25"?"SISAM":"SINASC"}<span>
</center>
</div>
</div>`
}
...
${(()=>{let s=d3.create("select").attr("class","menuIG");
            ...
            s.append("option").attr("name","PESO AO NASCER").text("PESO AO NASCER");
            s.append("option").attr("name","GRUPOS DE ROBSON").text("GRUPOS DE ROBSON")
            s.append("option").attr("name","TOTAL").text("NASCIMENTOS TOTAIS")
...
function _addTrilha(d3,conf_trilhas,$0,sel_municipios,fetchPM25,$1,procPM25,ampliaMenu,corTrilha,listenerFiltro,fetchPESO,procPESO,fetchRobson,procRobson,fetchTOTAL,procTOTAL){return(
(e)=>{
  ...
  Promise.all([fetchPM25(m.nome)]).then(v=>{
    ...
    case ("PESO AO NASCER"):
      conf={i:lastIdx+1, mostra_media_movel:false, variavel:"PESO", alterar_tempo:null, filtros:new ampliaMenu.local(null,null,corTrilha["PESO"]), show_filtros:false}
      conf.filtros.addEventListener("input",e=>listenerFiltro(e,conf))
      $0.value=conf_trilhas.concat(conf)
        sel_municipios.forEach(m=>Promise.all([fetchPESO(m.nome,conf.filtros.expression)]).then(v=>{
    $1.value=$1.value.concat([{idx:lastIdx+1,municipio:m.nome,variavel:"PESO",dados:procPESO(v[0])}])
  }))
      break
      
    case ("GRUPOS DE ROBSON"):
      conf={i:lastIdx+1, mostra_media_movel:false, escala:"taxa", robson:"Robson1a4", variavel:"ROBSON", alterar_tempo:null, filtros:new ampliaMenu.local(null,null,corTrilha["ROBSON"]), show_filtros:false}
      conf.filtros.addEventListener("input",e=>listenerFiltro(e,conf))
      $0.value=conf_trilhas.concat(conf)
      sel_municipios.forEach(m=>Promise.all([fetchRobson(m.nome,conf.filtros.expression)]).then(v=>{
    $1.value=$1.value.concat([{idx:lastIdx+1,municipio:m.nome,variavel:"ROBSON",dados:procRobson(v[0])}])
          }))
      break
 
    case ("NASCIMENTOS TOTAIS"):
...
function _checkRobson(conf_trilhas,$0){return(
(e,idx)=>{
  let temp=conf_trilhas
  conf_trilhas.find(d=>d.i==idx).robson=e.target.value
  $0.value=temp
}
)}
  ...
  if (urlParamConf) {
    ...
    return [
      ...
      },
      {
        ...
        variavel: "PESO",
        alterar_tempo: null,
        filtros: new ampliaMenu.local(null,null,corTrilha["PESO"]),
        show_filtros: false
      },
      {
        i: 2,
        ...
        variavel: "ROBSON",
        alterar_tempo: null,
        filtros: new ampliaMenu.local(null,null,corTrilha["ROBSON"]),
        show_filtros: false
      }
    ];
}
...
{PM25:{valor:"µg/m³"},PESO:{valor:"g"},ROBSON:{absoluto:"nascimentos",percentual:"%",taxa:"nasc./1000NV"},TOTAL:{absoluto:"nascimentos",percentual:"%"}}
)}
...
function _onfirstload(firstload,$0,$1,conf_trilhas,funcoesFetch,mp,$2,funcoesProc,listenerFiltro,listenerFiltroGlobal,sessionStorage,getCurrentConf,$3,$4)
{
  if (firstload) {
    ...
    mp.addEventListener("aba",e=>{
      sessionStorage.setItem("conf2",JSON.stringify(getCurrentConf()))
...
function _queryLocalSINASC(agregacoes){return(
(municipio) => {
  if (municipio=="BRASIL")
    return ""
  if (agregacoes.includes(municipio))
    return "AND res_SIGLA_UF='"+municipio+"'"
  return "AND res_MUNNOMEX='"+municipio.normalize("NFD").replace(/\p{Diacritic}/gu, "")+"'"
}
)}
...
function _defaultConf(){return(
{
  ...
  trilhas: [
    ...
    {i: 1, mostra_media_movel: false, variavel: "PESO", alterar_tempo: null, show_filtros: false, filtrosUrlids: ""},
    {i: 2, mostra_media_movel: false, escala: "taxa", robson: "Robson1a4", variavel: "ROBSON", alterar_tempo: null, show_filtros: false, filtrosUrlids: ""}
  ]
}
)}
...
function _urlParamConf(URLSearchParams,urlParamToObj,sessionStorage,defaultConf)
{
  ...
  urlParam = sessionStorage.getItem("conf2"); //loads from previous visit to this tab
  if (urlParam) {
...
"https://ampliasaude.github.io/trilhas5-2.html"
)}
...
function _fetchPESO(agregacoes,municipios,newDataSource){return(
async (municipio,filtros) => {
  ...
  return await newDataSource.fetchPeso(codMunicipio,filtros);
}
)}

function _procPESO(semanas_epidemiologicas){return(
(d)=>{
  d.forEach(row=>{
    let dict=row;
  ...
  for (let i of ret){
    ...
    if(f){
      f.valor+=i.valor*i.total;
      f.total+=i.total;
    }
    else
      ret2.push({x:i.x, valor:i.valor*i.total, total:i.total});
  }
  ret2=ret2.map(d=>{d.valor/=d.total; delete d.total; return d;}).filter(d=>d.x);
  return ret2;
}
)}

function _fetchRobson(agregacoes,municipios,newDataSource){return(
async (municipio,filtros) => {
  let codMunicipio
  if (agregacoes.includes(municipio))
    codMunicipio = Math.floor(municipios.find(d=>d.sigla==municipio).codigo_sdv/10000)
    ...
    codMunicipio = municipios.find(d=>d.municipio==municipio).codigo_sdv;
  return await newDataSource.fetchRobson(codMunicipio,filtros);
}
)}

function _procRobson(procTOTALUnfiltered,semanas_epidemiologicas){return(
(d) => {
  ...
  for (let i of ret){
    let f=ret2.find(d=>d.x.getTime()==i.x.getTime())
    if (f){
      f.absoluto[i.robson]+=i.total
      f.total+=i.total
    }
    else
      ret2.push({x:i.x, absoluto:{Robson1a4:(i.robson=="Robson1a4")?i.total:0,Robson5:(i.robson=="Robson5")?i.total:0,Robson6a10:(i.ig=="Robson6a10")?i.total:0}, total:i.total})
  }
  semanas_epidemiologicas.filter(d=>!ret2.map(e=>e.x.getTime()).includes(d.inicio.getTime())).forEach(d=>{
    ret2.push({x:d.inicio,absoluto:{Robson1a4:0,Robson5:0,Robson6a10:0},total:0})
  })

  ret2=ret2.map(d=>{
    d.percentual={}
    if (d.total){
      d.percentual["Robson1a4"]=100*d.absoluto["Robson1a4"]/d.total
      d.percentual["Robson5"]=100*d.absoluto["Robson5"]/d.total
      d.percentual["Robson6a10"]=100*d.absoluto["Robson6a10"]/d.total
    }
    else{
      d.percentual["Robson1a4"]=0
      d.percentual["Robson5"]=0
      d.percentual["Robson6a10"]=0
    }
    d.taxa={}
    let total=t.find(t2=>t2.x.getTime()==d.x.getTime())
    if (total){
      d.taxa["Robson1a4"]=1000*d.absoluto["Robson1a4"]/total.valor
      d.taxa["Robson5"]=1000*d.absoluto["Robson5"]/total.valor
      d.taxa["Robson6a10"]=1000*d.absoluto["Robson6a10"]/total.valor
    }
    else{
      d.taxa["Robson1a4"]=0
      d.taxa["Robson5"]=0
...
function _fetchTOTAL(agregacoes,municipios,newDataSource){return(
async (municipio,filtros) => {
  let codMunicipio
  if (agregacoes.includes(municipio))
    codMunicipio = Math.floor(municipios.find(d=>d.sigla==municipio).codigo_sdv/10000)
    ...
    codMunicipio = municipios.find(d=>d.municipio==municipio).codigo_sdv;
  return await newDataSource.fetchTotal(codMunicipio,filtros);
}
)}
...
function(min, max, starting_min=min, starting_max=max, width2=layout.width,margin_left=60) {
  ...
  var brush = d3_5.brushX()
    ...
    .on('brush', function() {
      ...
      //mutable sel=Math.max(Math.min(new Date(mutable sel),semanas_epidemiologicas[svg.node().value[1]-2].termino),semanas_epidemiologicas[svg.node().value[0]].inicio);
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("btn_adicionar_municipio")).define("btn_adicionar_municipio", ["d3","municipios","sel_municipios","mutable sel_municipios","colors","cache","mutable data","data","fetchPM25","fetchPESO","fetchRobson","procPM25","procPESO","procRobson"], _btn_adicionar_municipio);
  ...
  main.variable(observer("funcoesGeradoras")).define("funcoesGeradoras", ["gPM25","gPESO","gROBSON","gTOTAL"], _funcoesGeradoras);
  main.variable(observer("funcoesProc")).define("funcoesProc", ["procPM25","procPESO","procRobson","procTOTAL"], _funcoesProc);
  main.variable(observer("funcoesFetch")).define("funcoesFetch", ["fetchPM25","fetchPESO","fetchRobson","fetchTOTAL"], _funcoesFetch);
  ...
  main.variable(observer("gPESO")).define("gPESO", ["sel_municipios","data","gLoadingTrilha","conf_trilhas","mp","Plot","width","height","semanas_epidemiologicas","intervalo","filterDate","janela","lineWidthFiltered","lineWidth","bisector","sel","legendTooltip","mutable sel","d3","mouseDisplace","html","corTrilha","removeTrilha","copy_button","checkMediaMovel","checkTempo","inner_slider","checkFiltros"], _gPESO);
  main.variable(observer("gROBSON")).define("gROBSON", ["sel_municipios","data","gLoadingTrilha","conf_trilhas","mp","Plot","width","height","semanas_epidemiologicas","intervalo","filterDate","janela","lineWidthFiltered","lineWidth","bisector","sel","legendTooltip","mutable sel","d3","mouseDisplace","html","corTrilha","removeTrilha","copy_button","checkRobson","checkMediaMovel","checkTempo","checkEscala","inner_slider","checkFiltros"], _gROBSON);
  ...
  main.variable(observer("addTrilha")).define("addTrilha", ["d3","conf_trilhas","mutable conf_trilhas","sel_municipios","fetchPM25","mutable data","procPM25","ampliaMenu","corTrilha","listenerFiltro","fetchPESO","procPESO","fetchRobson","procRobson","fetchTOTAL","procTOTAL"], _addTrilha);
  ...
  main.variable(observer("checkRobson")).define("checkRobson", ["conf_trilhas","mutable conf_trilhas"], _checkRobson);
  ...
  main.variable(observer("queryLocalSINASC")).define("queryLocalSINASC", ["agregacoes"], _queryLocalSINASC);
  ...
  main.variable(observer("fetchPESO")).define("fetchPESO", ["agregacoes","municipios","newDataSource"], _fetchPESO);
  main.variable(observer("procPESO")).define("procPESO", ["semanas_epidemiologicas"], _procPESO);
  main.variable(observer("fetchRobson")).define("fetchRobson", ["agregacoes","municipios","newDataSource"], _fetchRobson);
  main.variable(observer("procRobson")).define("procRobson", ["procTOTALUnfiltered","semanas_epidemiologicas"], _procRobson);
  main.variable(observer("procTOTALUnfiltered")).define("procTOTALUnfiltered", ["semanas_epidemiologicas"], _procTOTALUnfiltered);
  main.variable(observer("fetchTOTAL")).define("fetchTOTAL", ["agregacoes","municipios","newDataSource"], _fetchTOTAL);
  ...
  main.import("menuCaracteristicasNascimento", "ampliaMenu", child1);
  main.import("datasource", "newDataSource", child1);

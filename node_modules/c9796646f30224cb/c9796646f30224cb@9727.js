...
md`# Amplia Saúde - Trilhas Exploratórias - v4.0 - Nascidos Vivos`
)}
...
html`${sel_municipios.map((d,i)=>remove_mun_button(d,i))}${sel_municipios.length<maxMunicipios?menu_municipios:""}`
)}
...
function _funcoesGeradoras(gPM25,gPESO,gPIG,gTOTAL){return(
{PM25:gPM25,PESO:gPESO,PIG:gPIG,TOTAL:gTOTAL}
)}

function _funcoesProc(procPM25,procPESO,procPIG,procTOTAL){return(
{PM25:procPM25,PESO:procPESO,PIG:procPIG,TOTAL:procTOTAL}
)}

function _funcoesFetch(fetchPM25,fetchPESO,fetchPIG,fetchTOTAL){return(
{PM25:fetchPM25,PESO:fetchPESO,PIG:fetchPIG,TOTAL:fetchTOTAL}
)}
...
<div class="drag-container"> <span class="material-icons-outlined ${conf_trilhas[0].i==idx?"icon-gray":"icon-blue"}">
expand_less
</span>
<div class="drag">
  <span class="material-icons-outlined icon-blue">
...
function _gPIG(sel_municipios,data,gLoadingTrilha,conf_trilhas,mp,Plot,width,height,semanas_epidemiologicas,intervalo,filterDate,janela,lineWidthFiltered,lineWidth,bisector,sel,legendTooltip,$0,d3,mouseDisplace,html,corTrilha,removeTrilha,copy_button,checkTamanho,checkMediaMovel,checkTempo,checkEscala,inner_slider,checkFiltros){return(
(idx) => {
  if(!sel_municipios.map(d=>d.nome).every(d=>data.filter(e=>e.idx==idx).map(e=>e.municipio).includes(d)))
    return gLoadingTrilha()
  ...
  let tamanho=conf_trilhas.find(d=>d.i==idx).tamanho
  let g=
    Plot.plot({
    marginTop:5,
    marginRight:0,
...
data.filter(d=>d.idx==idx).map((d,i)=>Plot.line(d.dados.filter(d=>filterDate(d,idx)),Plot.windowY({
  ...
  strokeDasharray:d.filtrado?"3":"1"
      ...
      :data.filter(d=>d.idx==idx).map((d,i)=>Plot.line(d.dados.filter(d=>filterDate(d,idx)), {
        ...
        Plot.ruleX([data.filter(d=>d.idx==idx)[0].dados[bisector.center(data.filter(d=>d.idx==idx)[0].dados,sel)]],{x:"x",strokeDasharray:"5"}),
        !conf_trilhas.find(d=>d.i==idx).mostra_media_movel?data.filter(d=>d.idx==idx).map((d,i)=>Plot.dot([d.dados[bisector.center(d.dados,sel)]],{x:"x",y:d=>d[escala][tamanho],fill:sel_municipios.find(e=>e.nome==d.municipio).cor})):null
      ]
    })

   legendTooltip(g,'PIG',idx,escala,tamanho);

  g.onmousedown=(e)=>{$0.value=Math.min(Math.max(g.scale("x").invert(e.offsetX),g.scale("x").domain[0]),g.scale("x").domain[1])}
  d3.select(g).call(d3.drag().on("drag",(e)=>$0.value=Math.min(Math.max(g.scale("x").invert(e.x-mouseDisplace(e.x)),g.scale("x").domain[0]),g.scale("x").domain[1])))

    return html`
<div style="--cor-trilha: ${corTrilha["PIG"]}; display:inline-flex; align-items:center; justify-content:space-between;">
<div class="buttons">
  ${d3.create("button").attr("class","btn-remove").text("–").on("click",removeTrilha).node()}<br>
  ${copy_button()}
</div>
...
<div class="trilha" i=${idx}>
  <div class="cabecalho-trilha">
    ...
    ${(()=>{let s=d3.create("select").attr("class","menuIG");
            ...
            s.append("option").attr("name","PEQUENO").text("PEQUENO").property("selected",conf_trilhas.find(d=>d.i==idx).tamanho=="PEQUENO");
            s.append("option").attr("name","ADEQUADO").text("ADEQUADO").property("selected",conf_trilhas.find(d=>d.i==idx).tamanho=="ADEQUADO");
            s.append("option").attr("name","GRANDE").text("GRANDE").property("selected",conf_trilhas.find(d=>d.i==idx).tamanho=="GRANDE");
            s.on("change",checkTamanho);
            return s.node();}
      )()}
...
${(()=>{let s=d3.create("select").attr("class","menuIG");
            ...
            s.append("option").attr("name","ADEQUAÇÃO À IDADE GESTACIONAL").text("ADEQUAÇÃO À IDADE GESTACIONAL")
...
function _addTrilha(d3,conf_trilhas,$0,sel_municipios,fetchPM25,$1,procPM25,ampliaMenu,corTrilha,listenerFiltro,fetchPESO,procPESO,fetchPIG,procPIG,fetchTOTAL,procTOTAL){return(
(e)=>{
  ...
  Promise.all([fetchPM25(m.nome)]).then(v=>{
    ...
    case ("ADEQUAÇÃO À IDADE GESTACIONAL"):
      conf={i:lastIdx+1, mostra_media_movel:false, escala:"taxa", tamanho:"PEQUENO", variavel:"PIG", alterar_tempo:null, filtros:new ampliaMenu.local(null,null,corTrilha["PIG"]), show_filtros:false}
      conf.filtros.addEventListener("input",e=>listenerFiltro(e,conf))
      $0.value=conf_trilhas.concat(conf)
      sel_municipios.forEach(m=>Promise.all([fetchPIG(m.nome,conf.filtros.expression)]).then(v=>{
    $1.value=$1.value.concat([{idx:lastIdx+1,municipio:m.nome,variavel:"PIG",dados:procPIG(v[0])}])
          }))
      break
...
function _checkTamanho(conf_trilhas,$0){return(
(e)=>{
  let temp=conf_trilhas
  conf_trilhas.find(d=>d.i==e.target.parentElement.parentElement.getAttribute("i")).tamanho=e.target.value
  $0.value=temp
}
)}
  ...
  if (urlParamConf) {
    ...
    return [
      ...
      },
      {
        ...
        variavel: "PIG",
        alterar_tempo: null,
        filtros: new ampliaMenu.local(null,null,corTrilha["PIG"]),
        show_filtros: false
      }
    ];
}
...
{PM25:{valor:"µg/m³"},PESO:{valor:"g"},PIG:{absoluto:"nascimentos",percentual:"%",taxa:"nasc./1000NV"},TOTAL:{absoluto:"nascimentos",percentual:"%"}}
)}
...
function _onfirstload(firstload,$0,$1,conf_trilhas,funcoesFetch,mp,$2,funcoesProc,listenerFiltro,listenerFiltroGlobal,sessionStorage,getCurrentConf,$3,$4)
{
  if (firstload) {
    ...
    mp.addEventListener("aba",e=>{
      sessionStorage.setItem("conf",JSON.stringify(getCurrentConf()))
...
function _sqlSinasc($0,municipios,SQLiteDatabaseClient){return(
function sqlSinasc(municipio,qry){
  $0.value = true;
  let codMunicipio = municipios.find(d=>d.municipio==municipio).codigo_sdv;
  let codUf = codMunicipio.substring(0,2);
  let url = `https://storage.googleapis.com/amplia/municipios-v2/${codUf}/mun${codMunicipio}.db`;
...
function _defaultConf(){return(
{
  ...
  trilhas: [
    ...
    {i: 2, mostra_media_movel: false, escala: "taxa", tamanho: "PEQUENO", variavel: "PIG", alterar_tempo: null, show_filtros: false, filtrosUrlids: ""}
  ]
}
)}
...
function _urlParamConf(URLSearchParams,urlParamToObj,sessionStorage,defaultConf)
{
  ...
  urlParam = sessionStorage.getItem("conf"); //loads from previous visit to this tab
  if (urlParam) {
...
"https://ampliasaude.github.io/trilhas5.html"
)}
...
function _fetchPIG(agregacoes,municipios,newDataSource){return(
async (municipio,filtros) => {
  let codMunicipio
  if (agregacoes.includes(municipio))
    codMunicipio = Math.floor(municipios.find(d=>d.sigla==municipio).codigo_sdv/10000)
    ...
    codMunicipio = municipios.find(d=>d.municipio==municipio).codigo_sdv
  return await newDataSource.fetchPIG(codMunicipio,filtros);
}
)}

function _procPIG(procTOTALUnfiltered,semanas_epidemiologicas){return(
(d) => {
  ...
  for (let i of ret){
    let f=ret2.find(d=>d.x.getTime()==i.x.getTime())
    if (f){
      f.absoluto[i.ig]+=i.total
      f.total+=i.total
    }
    else
      ret2.push({x:i.x, absoluto:{PEQUENO:(i.ig=="PEQUENO")?i.total:0,ADEQUADO:(i.ig=="ADEQUADO")?i.total:0,GRANDE:(i.ig=="GRANDE")?i.total:0}, total:i.total})
  }
  semanas_epidemiologicas.filter(d=>!ret2.map(e=>e.x.getTime()).includes(d.inicio.getTime())).forEach(d=>{
    ret2.push({x:d.inicio,absoluto:{PEQUENO:0,ADEQUADO:0,GRANDE:0},total:0})
  })

  ret2=ret2.map(d=>{
    d.percentual={}
    if (d.total){
      d.percentual["PEQUENO"]=100*d.absoluto["PEQUENO"]/d.total
      d.percentual["ADEQUADO"]=100*d.absoluto["ADEQUADO"]/d.total
      d.percentual["GRANDE"]=100*d.absoluto["GRANDE"]/d.total
    }
    else{
      d.percentual["PEQUENO"]=0
      d.percentual["ADEQUADO"]=0
      d.percentual["GRANDE"]=0
    }
    d.taxa={}
    let total=t.find(t2=>t2.x.getTime()==d.x.getTime())
    if (total){
      d.taxa["PEQUENO"]=1000*d.absoluto["PEQUENO"]/total.valor
      d.taxa["ADEQUADO"]=1000*d.absoluto["ADEQUADO"]/total.valor
      d.taxa["GRANDE"]=1000*d.absoluto["GRANDE"]/total.valor
    }
    else{
      d.taxa["PEQUENO"]=0
      d.taxa["ADEQUADO"]=0
  ...
  return ret2.sort((a,b)=>a.x.getTime()-b.x.getTime())
}
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("funcoesGeradoras")).define("funcoesGeradoras", ["gPM25","gPESO","gPIG","gTOTAL"], _funcoesGeradoras);
  main.variable(observer("funcoesProc")).define("funcoesProc", ["procPM25","procPESO","procPIG","procTOTAL"], _funcoesProc);
  main.variable(observer("funcoesFetch")).define("funcoesFetch", ["fetchPM25","fetchPESO","fetchPIG","fetchTOTAL"], _funcoesFetch);
  ...
  main.variable(observer("addTrilha")).define("addTrilha", ["d3","conf_trilhas","mutable conf_trilhas","sel_municipios","fetchPM25","mutable data","procPM25","ampliaMenu","corTrilha","listenerFiltro","fetchPESO","procPESO","fetchPIG","procPIG","fetchTOTAL","procTOTAL"], _addTrilha);
  ...
  main.variable(observer("checkTamanho")).define("checkTamanho", ["conf_trilhas","mutable conf_trilhas"], _checkTamanho);
  ...
  main.variable(observer("sqlSinasc")).define("sqlSinasc", ["mutable loading","municipios","SQLiteDatabaseClient"], _sqlSinasc);
  ...
  main.variable(observer("fetchPIG")).define("fetchPIG", ["agregacoes","municipios","newDataSource"], _fetchPIG);
  main.variable(observer("procPIG")).define("procPIG", ["procTOTALUnfiltered","semanas_epidemiologicas"], _procPIG);
  ...
  main.import("menuNascidosVivos", "ampliaMenu", child1);
